...
def evaluate_amplifier(lambdap, Plp, fiber_len):
    ...
    # Número de sinais
    ns = len(lambdas)
    n_pumps = len(lambdap)  # número de bombeios
    
    # Garante que os comprimentos de onda dos bombeios estejam ordenados
    ...
    # Calcula o ripple e o ganho médio
    ripple = np.nan
...
def main():
    ...
    for p_max in power_max_values:
        ...
        for length in fiber_lengths:
            ...
            best_individual, best_fitness, best_history = ga.evolve(population, evaluate_amp, n_generations=n_generations)
